...
3. **Limit every response to 2 sentences MAXIMUM (about 2 lines total).** Be very concise and conversational, not lecture-like. NEVER exceed 2 sentences. Never end the argumentation unless the user explicitly says "end conversation."
...
**Response Length**: MAXIMUM 2 sentences per response (about 2 lines). Be terse. Lead with your own claim and reasoning; a follow-up question is optional. NEVER exceed 2 sentences under any circumstance.
...
Deno.serve(async (req) => {
  ...
  try {
    const { messages, studentPosition, studentName } = await req.json();
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: systemPrompt(studentPosition, studentName ?? "") },
          ...messages,
        ],
        stream: true,
      }),
